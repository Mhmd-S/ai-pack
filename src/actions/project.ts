...
export const createProject = async (
    ...
    formData: FormData
): Promise<ProjectFormState> => { // Ensure the return type matches
    try {
        ...
        generateTexture({
            ...
            console.error('Background texture generation error:', textureError);
            Project.findByIdAndUpdate(project._id, { // Fire and forget update
                status: 'error',
            }).catch(dbError => console.error('Error updating project status after texture error:', dbError));
        });
